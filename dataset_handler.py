...
class DatasetHandler(object):
    ...
    def load_movies(self):
        movies_frame = pd.read_csv(os.path.join(self.dataset_path, "movies.csv"), names=["movieId", "title", "genres"], sep=",", engine="python",header=0)
        self.id_to_title = {}
    ...
    def load_users_ratings(self):
        ratings_frame = pd.read_csv(os.path.join(self.dataset_path, "ratings.csv"), names=["userId", "movieId", "rating", "timestamp"], sep=",", engine="python",header=0)
        users_ratings = {}
        print('starting to load user ratings')
        for _, row in tqdm(ratings_frame.iterrows()):
